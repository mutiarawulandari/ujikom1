...
if ($pemilik_id <= 0) {
    $_SESSION['error'] = "Anda belum login sebagai pemilik. Silakan login terlebih dahulu.";
...
if (!$checkUser || mysqli_num_rows($checkUser) == 0) {
    $_SESSION['error'] = "Data pemilik tidak ditemukan di database. Hubungi admin.";
...
if (!is_dir($uploadDir)) {
    ...
    if (!mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
        $_SESSION['error'] = "Gagal membuat folder upload. Pastikan permission folder web server memungkinkan membuat folder.";
        header("Location: ../index.php");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            // Insert motor
            $sql = "INSERT INTO motor (pemilik_id, merk, tipe_cc, no_plat, photo, dokumen_kepemilikan, status) 
                    VALUES (?, ?, ?, ?, ?, ?, 'pending')";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "isssss", $pemilik_id, $merk, $tipe_cc, $no_plat, $photoFile, $dokumenFile);
            mysqli_stmt_execute($stmt);
            $motor_id = mysqli_insert_id($conn);
      ...
      // Set session success
$_SESSION['success'] = "Motor berhasil ditambahkan. Motor akan muncul di dashboard setelah diverifikasi oleh admin.";

// Redirect ke daftar motor
header("Location: index.php");
exit;
        } catch (Exception $e) {
            // Rollback jika ada error
            mysqli_rollback($conn);
            $errors[] = "Gagal membuat motor: " . $e->getMessage();
        }
    }
}
?>
